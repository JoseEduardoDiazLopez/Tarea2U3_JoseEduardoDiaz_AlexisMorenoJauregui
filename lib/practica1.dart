...
void actualizarTareas() async {
  ...
  // Filtrar las tareas para obtener solo las de hoy
  DateTime today = DateTime.now();
  String formattedDate = "${today.year}-${today.month}-${today.day}";
  List<Tarea> tempTareasHoy = tempT.where((tarea) => tarea.entrega == formattedDate).toList();
  print("Tareas de hoy: ${tempTareasHoy.length}"); // Agregar este print
  setState(() {
    data2 = tempT;
    data2Hoy = tempTareasHoy; // Guardar las tareas de hoy en una nueva lista
  });
}
  ...
  void initState(){
  actualizarLista();
  actualizarTareas(); // Esto carga todas las tareas
  actualizarTareasP();

  // Actualizar las tareas de hoy
  DateTime today = DateTime.now();
  String formattedDate = "${today.year}-${today.month}-${today.day}";
  List<Tarea> tempTareasHoy = data2.where((tarea) => tarea.entrega == formattedDate).toList();
  setState(() {
    data2Hoy = tempTareasHoy; // Guardar las tareas de hoy en data2Hoy
  });
}
  ...
  return Column(
    children: <Widget>[
      ...
      Expanded(child: ListView.builder(
          itemCount: data2Hoy.length, // Cambiar de data2 a data2Hoy
          itemBuilder: (context, indice){
            return Padding(
              ...
              child: Container(
                ...
                height: 75,
                child: ListTile(
                  title: Text("${data2Hoy[indice].descripcion}"),
                  subtitle: Text("Fecha de entrega: ${data2Hoy[indice].entrega}"),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      IconButton(
                        onPressed: (){
                          estGlobT=data2Hoy[indice];
                          actualizarT(data2Hoy[indice], indice);
                          setState(() {
                            actualizarTareas();
                          });
                        },
                      ...
                      IconButton(
                        onPressed: (){
                          DB.eliminarT(data2Hoy[indice].idtarea).then((value){
                            setState(() {
                              titulo="Se elimino correctamente👍🏻";
                            });
                            actualizarTareas();
      ...
      return Container(
        ...
        child: Form(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text("Agregar Tarea", style: TextStyle(fontSize: 25)),
              TextField(
                enabled: false,
                    ...
                    onPressed: () {
                      if (_formKey.currentState!.validate()) {
                        var tempT = Tarea(
                          ...
                          entrega: estado.text, // Asegúrate de que la fecha de entrega se esté obteniendo correctamente
                          descripcion: descripcion.text,
                        );
                        DB.insertarT(tempT).then((value) {
                          setState(() {
                            titulo = "Se insertó correctamente👍🏻";
                          });
                          idmateriat.text = "";
                          estado.text = "";
                          descripcion.text = "";
